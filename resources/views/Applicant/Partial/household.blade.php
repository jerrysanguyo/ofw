...
<form action="{{ route('admin.applicant.houseUpdate', $member->id) }}" method="POST">
    @csrf
    @method('PUT')
    <div class="row align-items-end mt-3">
        <div class="col-lg-3">
            <label for="full_name_{{ $index }}" class="form-label">Full name</label>
            <input type="text" name="full_name" id="full_name_{{ $index }}" class="form-control" value="{{ $member->full_name }}" required>
        </div>
        <div class="col-lg-3">
            <label for="relation_id_{{ $index }}" class="form-label">Relationship</label>
            <select name="relation_id" class="form-select" id="relation_id_{{ $index }}" required>
        ...
        <div class="col-lg-3">
            ...
            <input type="date" id="birthdate_{{ $index }}" name="birthdate" class="form-control birthdate-input" value="{{ $member->birthdate }}" required>
        </div>
        <div class="col-lg-2">
            <label for="age_{{ $index }}" class="form-label">Age</label>
            <input type="number" id="age_{{ $index }}" name="age" class="form-control age-input" value="{{ $member->age }}" min="0" max="255" required>
        </div>
        <div class="col-lg-3">
        ...
        <div class="col-lg-3">
            ...
            <select name="voters" id="voters" class="form-select" required>
